...
for($x =1 ; $x <= 35; $x++){//recorremos todos los alumnos,se recuperan cada uno de los datos del form siempre y cuando se hayan enviado, de lo contrario los omite
    ...
    $AlSECIONPF->setMESSAGENEURO(htmlentities($_REQUEST['txtnota1'.$x],ENT_QUOTES,"UTF-8"));
    $AlSECIONPF->GRABAR_NEURO();
}
    echo '<script>window.location = \'messageneuro.php\'</script>';
}
//-------------------------------------------------------------------------------------
?>
...
?>
<form id="frmmensaje" method="post" action="messageneuro.php?GRABAR=1">
<fieldset>
