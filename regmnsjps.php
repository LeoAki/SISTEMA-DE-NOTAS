...
<body>
<?php
require_once 'Includes/navegador.php';include_once 'Class/Conection.php';include_once 'Class/Docente.php';$DOCENTE= new Docente();
...
for($x =1 ; $x <= 35; $x++){//recorremos todos los alumnos,se recuperan cada uno de los datos del form siempre y cuando se hayan enviado, de lo contrario los omite
    $AlSECIONPF->setCODIGO($_REQUEST['txtcodigo'.$x]);    $AlSECIONPF->setCODIGOAS($_REQUEST['txtcodigo'.$x]);
    $AlSECIONPF->setCPF1($_REQUEST['txtnota1'.$x]);    $AlSECIONPF->setCPF2($_REQUEST['txtnota2'.$x]);
    $AlSECIONPF->setCPF3($_REQUEST['txtnota3'.$x]);    $AlSECIONPF->setCPF4($_REQUEST['txtnota4'.$x]);
    $AlSECIONPF->GRABAR();
}
...
<article>
<section>1. <i>Participaci&oacute;n en el desarrollo educativo de su hijo(a).</i></section><section>2. <i>Asistencia y puntualidad a las llamadas de la Instituci&oacute;n.</i></section><section>3. <i>Capacitaci&oacute;n en los Talleres-Escuela De Padres.</i></section><section>4. <i>Supervisi&oacute;n y acompa&ntilde;amiento a sus hijos para el cumplimiento del Reglamento Escolar.</i></section>
</article>
<?php
...
echo '<h5 style=\'color: peru\'>SECCION: '.$gradoname.' '.$nameseccionnow.' '.$nomnivelsection.'</h5><h5 style=\'color: peru\'>TUTOR: '.$apellidosdocentenow.'</h5>';
?>
<form id="frmmensaje" method="post" action="regmnsjps.php?GRABAR=1">
<fieldset><table class="table table-hover">
<tr class="gradeU">
    <td style="display: none;"></td><td style="width: 3%;color: peru;text-transform: uppercase;"><b>N</b></td><td style="width: 20%;color: peru;text-transform: uppercase;"><b>Alumno</b></td>
    <td style="width: 8%;color: peru;text-transform: uppercase;"><b>1</b></td><td style="width: 8%;color: peru;text-transform: uppercase;"><b>2</b></td>
    <td style="width: 8%;color: peru;text-transform: uppercase;"><b>3</b></td><td style="width: 8%;color: peru;text-transform: uppercase;"><b>4</b></td>
</tr>
<?
$whoisalumno=$AlSECIONPF->ALUMNOSDEMITUTORIA($codigodocentenow);
while ($row = mysql_fetch_array($whoisalumno)): ?>
 <tr><td style='display:none;'><?='<input type=\'text\' value=\''.$row[0].'\' id=\'txtcodigo'.$row[1].'\' name=\'txtcodigo'.$row[1].'\'/>';?></td><?php echo '<td><b>'.$row[1].'</b></td><td><b>'.$row[2].' '.$row[3].' ,</b>'.$row[4].'</td>';?>
<?='<td><input type=\'text\' value=\''.$row[29].'\' id=\'txtnota1'.$row[1].'\' name=\'txtnota1'.$row[1].'\' placeholder=\'FN\' style=\'width:100%;\' maxlength=\'2\'/></td>     <td><input type=\'text\' value=\''.$row[30].'\' id=\'txtnota2'.$row[1].'\' name=\'txtnota2'.$row[1].'\' placeholder=\'FN\' style=\'width:100%;\' maxlength=\'2\'/ ></td>     <td><input type=\'text\' value=\''.$row[31].'\' id=\'txtnota3'.$row[1].'\' name=\'txtnota3'.$row[1].'\' placeholder=\'FN\' style=\'width:100%;\' maxlength=\'2\' /></td>     <td><input type=\'text\' value=\''.$row[32].'\' id=\'txtnota4'.$row[1].'\' name=\'txtnota4'.$row[1].'\' placeholder=\'FN\' style=\'width:100%;\' maxlength=\'2\' /></td></tr>';
endwhile;?></table></fieldset>
