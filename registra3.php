...
<html>
    <head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8"><link rel="icon" href="Css/images/favicon.ico"><title>LNCC ONLINE ::: Registra Notas</title>
        ...
        <body>
            <?php require_once 'Class/Component.php';
            ...
            $_COOKIE["valuecombix"];
            ?> <center><h3 style="color: green">Registro De Notas por asignatura- III BIMESTRE</h3>
            <?PHP
            $asina = $_GET['sinatura'];
            ...
            if (isset($_REQUEST['GRABAR'])) {
                ...
                for ($x = 1; $x <= 35; $x++) {
                    ...
                    $REGISTROALUMNO->setP11($_REQUEST[$x . 'p11']);
                    $REGISTROALUMNO->setP12($_REQUEST[$x . 'p12']);
                    $REGISTROALUMNO->setP13($_REQUEST[$x . 'p13']);
                    $REGISTROALUMNO->setP14($_REQUEST[$x . 'p14']);
                    $REGISTROALUMNO->setP15($_REQUEST[$x . 'p15']);
                    $REGISTROALUMNO->setP16($_REQUEST[$x . 'p16']);
                    $REGISTROALUMNO->setP17($_REQUEST[$x . 'p17']);
                    $REGISTROALUMNO->setP18($_REQUEST[$x . 'p18']);
                    $REGISTROALUMNO->setP19($_REQUEST[$x . 'p19']);
                    $REGISTROALUMNO->setP110($_REQUEST[$x . 'p110']);
                    $REGISTROALUMNO->setP111($_REQUEST[$x . 'p111']);
                    $REGISTROALUMNO->setP112($_REQUEST[$x . 'p112']);
                    $REGISTROALUMNO->setP113($_REQUEST[$x . 'p113']);

                    $REGISTROALUMNO->setP21($_REQUEST[$x . 'p21']);
                    $REGISTROALUMNO->setP22($_REQUEST[$x . 'p22']);
                    $REGISTROALUMNO->setP23($_REQUEST[$x . 'p23']);
                    $REGISTROALUMNO->setP24($_REQUEST[$x . 'p24']);
                    $REGISTROALUMNO->setP25($_REQUEST[$x . 'p25']);
                    $REGISTROALUMNO->setP26($_REQUEST[$x . 'p26']);
                    $REGISTROALUMNO->setP27($_REQUEST[$x . 'p27']);
                    $REGISTROALUMNO->setP28($_REQUEST[$x . 'p28']);
                    $REGISTROALUMNO->setP29($_REQUEST[$x . 'p29']);
                    $REGISTROALUMNO->setP210($_REQUEST[$x . 'p210']);

                    $REGISTROALUMNO->setP31($_REQUEST[$x . 'p31']);
                    $REGISTROALUMNO->setP32($_REQUEST[$x . 'p32']);
                    $REGISTROALUMNO->setP33($_REQUEST[$x . 'p33']);
                    $REGISTROALUMNO->setP34($_REQUEST[$x . 'p34']);
                    $REGISTROALUMNO->setP35($_REQUEST[$x . 'p35']);
                    $REGISTROALUMNO->setP36($_REQUEST[$x . 'p36']);
                    $REGISTROALUMNO->setP37($_REQUEST[$x . 'p37']);
                    $REGISTROALUMNO->setP38($_REQUEST[$x . 'p38']);
                    $REGISTROALUMNO->setP39($_REQUEST[$x . 'p39']);
                    $REGISTROALUMNO->setP310($_REQUEST[$x . 'p310']);

                    $REGISTROALUMNO->setP41($_REQUEST[$x . 'p41']);
                    $REGISTROALUMNO->setP42($_REQUEST[$x . 'p42']);
                    $REGISTROALUMNO->setP43($_REQUEST[$x . 'p43']);
                    $REGISTROALUMNO->setP44($_REQUEST[$x . 'p44']);
                    $REGISTROALUMNO->setP45($_REQUEST[$x . 'p45']);
                    $REGISTROALUMNO->setP46($_REQUEST[$x . 'p46']);
                    $REGISTROALUMNO->setP47($_REQUEST[$x . 'p47']);
                    $REGISTROALUMNO->setP48($_REQUEST[$x . 'p48']);
                    $REGISTROALUMNO->setP49($_REQUEST[$x . 'p49']);
                    $REGISTROALUMNO->setP410($_REQUEST[$x . 'p410']);

                    $REGISTROALUMNO->setP51($_REQUEST[$x . 'p51']);
                    $REGISTROALUMNO->setP52($_REQUEST[$x . 'p52']);
                    $REGISTROALUMNO->setP53($_REQUEST[$x . 'p53']);
                    $REGISTROALUMNO->setP54($_REQUEST[$x . 'p54']);
                    $REGISTROALUMNO->setP55($_REQUEST[$x . 'p55']);
                    $REGISTROALUMNO->setP56($_REQUEST[$x . 'p56']);
                    $REGISTROALUMNO->setP57($_REQUEST[$x . 'p57']);
                    $REGISTROALUMNO->setP58($_REQUEST[$x . 'p58']);
                    $REGISTROALUMNO->setP59($_REQUEST[$x . 'p59']);
                    $REGISTROALUMNO->setP510($_REQUEST[$x . 'p510']);
                    
                    $REGISTROALUMNO->setP61($_REQUEST[$x . 'p61']);
                    $REGISTROALUMNO->setP71($_REQUEST[$x . 'p71']);
    ...
    echo'<center><h4>AULA[' . $variable1 . $nombredelaseccion . '] NIVEL[' . $variable2 . '] [Asignatura:' . $variable3 . ']</h4></center>';
    ?>
        <form name="frmregistro" method="post" action="registra3.php?GRABAR=0">
            <div style="background-color: greenyellow;"><center><b>LLENAR REGISTROS DE MANERA:</b><input type="Radio" name="group1"  value="horizontal" checked>Horizontal <input type="Radio" name="group1"  value="vertical" >Vertical<br><input type="Button" name="" value="Cambiar forma de navegaci&oacute;n" onclick="cambiaColor();
            window.location.reload()"><br><a>SE LLENARAN LOS REGISTROS DE MANERA : <b><?php echo $miVariable; ?></b></a><br><a>DOCENTE USA LA TECLA TAB PARA LA NAVEGACI&Oacute;N PORFAVOR</a></center>
            </div><br>
    ...
    while ($alumno = mysql_fetch_array($listaalumnado)) {
        ...
        <?php
        echo '<td style=\'width:25%;\'>' . $alumno[1] . ' ' . $alumno[2] . ',' . $alumno[3] . '</td>';
        ...
        while ($ro = mysql_fetch_array($td)) {
            ...
            while ($row22 = mysql_fetch_array($lista)) {
                ...
                while ($row11 = mysql_fetch_array($listadice)) {
                    ...
                    $valueespacio = $row11["3p$valorcelda"]; #valor de la celda
                    $promedio = round($row11["3promedio$ro[1]"]); #promedio por componente
                    $pbb = $row11['11']; #promedio final
                    if ($valueespacio == 0)
                        $valueespacio = "";#si no hay valor a espacio en blanco
                }
