...
<body>
<?
require_once 'Includes/navegador.php';  require_once 'Class/Component.php';
require_once 'Class/Indicador.php';     require_once 'Class/RegistroAlumnoInicial.php';
$INDICAXD= new Indicador();             $REGISTROALUMNO= new RegistroAlumnoInicial();
?><center><h3 style="color: green">Registro De Notas por asignatura- IV BIMESTRE :: INICIAL ::</h3></center>
<?
...
$listar=$COMPONENTE->LISTAR4($asina);
while ($row = mysql_fetch_array($listar)):
?><tr class='gradeX'></tr>
<?
$lista=$INDICAXD->LISTAR($row[0]);
    while ($row2 = mysql_fetch_array($lista)) {?>
        <tr class='gradeA'><td class='center'><a><?=$row[1].'.'.$row2[3]?></a></td><td><?=$row2[2]?></td></tr>
<?  }
endwhile;
?></table>
<?='<center><h4> '.$variable1.' '.$variable2.' ASIGNATURA: '.$variable3.' </h4></center>';?>
<form name="frmregistro" method="post" action="registrainicial4.php?GRABAR=0">
<center><table class="">
<thead><tr class="">
...
while ($roth = mysql_fetch_array($th)):
    ...
    while ($rowth = mysql_fetch_array($listath)) {
        ?><td class='center' style='width:3%;'> <?=$roth[1].'.'.$rowth[3]?></td><?
    }?><td style='color:peru;'>P<?=$roth[1]?></td><?
endwhile;
?><td style="color:peru;">PB</td>
</tr></thead>
<?
$reAl = new RegistroAlumnoInicial();    $listaalumnado=$reAl->ListaAlumnoSeccion($seccion);

while ($alumno = mysql_fetch_array($listaalumnado)):
?>
<tr>
<td><td><input type='hidden' name='txtregistro' id='txtregistro' value='<?=$registro?>'/></td>
...
<td style='width:3%;'><?=$alumno[0]?></td><td style='width:25%;'><?=$alumno[1].' '.$alumno[2].' ,'.$alumno[3];?></td>
<?
...
<td class='center' style="width:3%;">
 <input tabindex='<?=$row22[0]?>' placeholder='FN' type='text' id='<?=$alumno[0]?>p<?=$ro[1].$row22[3]?>' name='<?=$alumno[0]?>p<?=$ro[1].$row22[3]?>' value='<?=$valueespacio?>' style='width:89%;' maxlength=2; /></td>
<?      }#yissela_jeag@hotmail
?>
<td><input type='text' style='width:80%;'  value='<?=$valorpromedio?>' id='<?=$alumno[0]?>promedio<?=$ro[1]?>' name='<?=$alumno[0]?>promedio<?=$ro[1]?>' maxlength=2;/></td>
<?}
?><td><input type='text' style='width:80%;' id='<?=$alumno[0]?>pb' name='<?=$alumno[0]?>pb'  value='<?=$pb?>' maxlength=2/></td>
</tr>
<?endwhile;?>
</table></center>
<center>
<div class="form-actions"><button style="" type="submit"class="btn btn-primary" id="btnsavea" name="btnsavea">GRABAR O ACTUALIZAR NOTAS</button></div>
</center>
</form>
<? }?>
</body>
